#!/bin/bash
# 4 simultaneous Frigate-like decodes of camera-1296p-10s (hw = v4l2request ffmpeg in the frigate image, sw = image's own ffmpeg)
# usage: ./conc_test.sh hw|sw [N]
set -u
HERE=$(cd "$(dirname "$0")" && pwd); R=${RESEARCH:?set RESEARCH to the directory holding the test clips}; L=$HERE/logs; mode=$1; N=${2:-4}
if [ "$mode" = hw ]; then
...
start=$(date +%s.%N)
for i in $(seq 1 $N); do
  ( timeout -s KILL 25 $FF -i /r/camera-1296p-10s.h264 -vf fps=5,scale=640:360 -f rawvideo -pix_fmt yuv420p - 2>"$L/conc-$mode-$i.err" | md5sum > "$L/conc-$mode-$i.md5" ) &
done
wait
end=$(date +%s.%N)
