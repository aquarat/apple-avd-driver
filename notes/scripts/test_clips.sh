...
#   ./test_clips.sh                  run all f_*/g_*/t_*/h_*/camera-1296p-10s clips
...
RESEARCH=${RESEARCH:?set RESEARCH to the directory holding the test clips}
...
if [ ${#clips[@]} -eq 0 ]; then
  cd "$RESEARCH" && clips=( $(ls f_*.h264 g_*.h264 t_*.h264 h_*.h264 camera-1296p-10s.h264 2>/dev/null) )
fi
