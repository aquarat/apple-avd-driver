...
| 2. large / high-bitrate / CABAC frames stall | watchdog only, VP stops at 85-94 % of the slice, no error IRQ | **`final/0003`: T8103 `VP_INSN_FIFO_MASK` 0x100000 → 0** (backport of Janne Grunau's ca9a850f237f from 7.1.12) | g_1080_main, g_1080_high8x8, g_1296_base, g_1296_main, t_1080p_high, t_1080p_main_cavlc, t_1296p_main_cavlc, camera-1296p-10s → OK-identical |
...
`camera-1296p-10s.h264` (real IP-camera stream, 2304x1296 High, CABAC, 8x8, PPS scaling
...
| camera-1296p-10s (IP camera) | FAIL | FAIL | FAIL | OK (1) | n/t | n/t | OK | OK |
...
(1) camera-1296p-10s showed `FAIL(killed)` in the first 0004 run only because the
...
g_1296_base/h_1296_base_lowbr/camera-1296p-10s `_idr_only` and g_1080_main/g_1296_base/
camera-1296p-10s `_2frames` all OK-identical (they all FAILed on 0001+0002 except the
...
| camera-1296p-10s IDR | 218230 | 202426 | 93 |
...
| run | md5 | time |
|---|---|---|
| gst `v4l2slh264dec ! fakesink`, camera-1296p-10s 150 frames | — | 0.42-0.59 s wall (≈ 250-350 fps decode), 10 back-to-back runs all rc=0, dmesg silent |
...
### Concurrency (4 simultaneous decodes of camera-1296p-10s, Frigate-like filters, `conc_test.sh`)
...
| run (camera-1296p-10s, 150 frames, in `frigate:latest`) | utime | stime | rtime |
|---|---|---|---|
